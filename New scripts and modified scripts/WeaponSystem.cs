...
public class WeaponSystem : MonoBehaviour
{
    ...
    private bool setWepTo1OncOnly;
    private bool setWepTo2OncOnly;
    ...
    void Start()
    {
        ...
        setWepTo1OncOnly = true;
        setWepTo2OncOnly = true;
    }
    ...
    void Update()
    {
        ...
        if (Input.GetAxis("Mouse ScrollWheel") > 0)
            SetActiveWeapon(weaponIndex);
        if (Input.GetAxis("Mouse ScrollWheel") < 0)
            SetActiveWeapon(weaponIndex-1);
        if ((weaponIndex==1)&(setWepTo1OncOnly))
        {
            SetActiveWeapon(weaponIndex);
            setWepTo1OncOnly = false;
        }
        if ((weaponIndex == 2) & (setWepTo2OncOnly))
        {
            SetActiveWeapon(weaponIndex);
            setWepTo2OncOnly = false;
        }
    }
